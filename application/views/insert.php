...
<?php 
if($_POST["table"] == "list_anggota_keluarga" && $_POST["media"] == "edit")
{
	unset($_POST["table"]);
	unset($_POST["media"]);
	...
	$this->db->where("id", $_COOKIE["id_edit"]);

	if($this->db->update("list_anggota_keluarga", $_POST))
	{
		setcookie("pesan_insert", "Berhasil Melakukan Edit Data !", time() + 1000000, "/");
		header("Location: list_anggota_keluarga_admin?id_anggota_keluarga=$_COOKIE[id_anggota_keluarga_edit]");
	}
	else
	{
